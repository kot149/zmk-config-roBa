# キーマップ
自分の考えるキーマップについて語り、その設定方法を紹介する。
なお、ZMK(roBa)を中心に書くが、一部QMK(Keyball)や、AutoHotKeyやKarabiner Elements(非カスタムキーボード向け)の話も交える。
...
そのため、実は「マウスキーを押したらタイムアウトを延長する」と「マウスキーを押すまでマウスレイヤーに留まる」は同じである。
