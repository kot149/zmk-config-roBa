...
if [ "$FULL_BUILD" = true ]; then
    # フルビルド用のコマンド
    RIGHT_BUILD_CMD="west build -p -s app -d build/right -b seeeduino_xiao_ble -S studio-rpc-usb-uart -- -DZMK_CONFIG=/workspaces/zmk-config/config -DSHIELD=\"roBa_R rgbled_adapter\" -DZMK_EXTRA_MODULES=$EXTRA_MODULES_STR_RIGHT"
    LEFT_BUILD_CMD="west build -p -s app -d build/left  -b seeeduino_xiao_ble -- -DZMK_CONFIG=/workspaces/zmk-config/config -DSHIELD=\"roBa_L rgbled_adapter\" -DZMK_EXTRA_MODULES=$EXTRA_MODULES_STR_LEFT"
